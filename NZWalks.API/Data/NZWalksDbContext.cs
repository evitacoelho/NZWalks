using Microsoft.EntityFrameworkCore;
using NZWalks.API.Models.Domain;

namespace NZWalks.API.Data
{
    public class NZWalksDbContext : DbContext 
    {
        // this is used by the controllers to talk to the DB
        //pass the db context options to the base class
        //when using multiple DbContext provide the Context of each DbContext
        public NZWalksDbContext(DbContextOptions <NZWalksDbContext> dbContextOptions) : base (dbContextOptions)
        {
            
        }
        //DbSet represents a collection of entities in the DB i.e. domain models (Walk, Region, Difficulty)
        //Naming Convention: Prop names are plural of the domain name - creates tables within the DB based on the domain model
        public DbSet<Difficulty> Difficulties { get; set; }
        public DbSet<Region> Regions { get; set; }
        public DbSet<Walk> Walks { get; set; }


        //seeding the DB
        protected override void OnModelCreating(ModelBuilder modelBuilder)
        {
            base.OnModelCreating(modelBuilder);

            //seed data for difficulty - E, M, H
            //create a list of difficulty levels
            var difficulties = new List<Difficulty>()
            {
                new Difficulty()
                {
                    //generated using C# interactive window to get a constant guid value
                    Id = Guid.Parse ("b0c1c1a2-91ab-459d-8dce-211757b1dbfd"),
                    Name = "Easy"
                },
                new Difficulty()
                {
                    //generated using C# interactive window to get a constant guid value
                    Id = Guid.Parse ("8dc34bc3-0cc9-4b2d-893d-12fa040a57c7"),
                    Name = "Medium"
                },
                new Difficulty()
                {
                    //generated using C# interactive window to get a constant guid value
                    Id = Guid.Parse ("4a9bf982-b69b-4b6e-99ec-cbbf50cc4acd"),
                    Name = "Hard"
                }
            };
            //seed the difficulty domain model with the defined data
            modelBuilder.Entity<Difficulty>().HasData(difficulties);

            //seed data for Regions
            var regions = new List<Region>
            {
                new Region
                {
                    Id = Guid.Parse("f7248fc3-2585-4efb-8d1d-1c555f4087f6"),
                    Name = "Auckland",
                    Code = "AKL",
                    RegionImageUrl = "https://images.pexels.com/photos/5169056/pexels-photo-5169056.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                },
                new Region
                {
                    Id = Guid.Parse("6884f7d7-ad1f-4101-8df3-7a6fa7387d81"),
                    Name = "Northland",
                    Code = "NTL",
                    RegionImageUrl = null
                },
                new Region
                {
                    Id = Guid.Parse("14ceba71-4b51-4777-9b17-46602cf66153"),
                    Name = "Bay Of Plenty",
                    Code = "BOP",
                    RegionImageUrl = null
                },
                new Region
                {
                    Id = Guid.Parse("cfa06ed2-bf65-4b65-93ed-c9d286ddb0de"),
                    Name = "Wellington",
                    Code = "WGN",
                    RegionImageUrl = "https://images.pexels.com/photos/4350631/pexels-photo-4350631.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                },
                new Region
                {
                    Id = Guid.Parse("906cb139-415a-4bbb-a174-1a1faf9fb1f6"),
                    Name = "Nelson",
                    Code = "NSN",
                    RegionImageUrl = "https://images.pexels.com/photos/13918194/pexels-photo-13918194.jpeg?auto=compress&cs=tinysrgb&w=1260&h=750&dpr=1"
                },
                new Region
                {
                    Id = Guid.Parse("f077a22e-4248-4bf6-b564-c7cf4e250263"),
                    Name = "Southland",
                    Code = "STL",
                    RegionImageUrl = null
                },
            };
            //seed the Region domain model with the defined data
            modelBuilder.Entity<Region>().HasData(regions);


        }

    }
}
